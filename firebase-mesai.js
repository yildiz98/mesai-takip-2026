...
function setAuthMode(mode) {
  authMode = mode;
  clearAuthError();
  document.getElementById("nameWrap").style.display = mode === "register" ? "block" : "none";
  document.getElementById("authSubmit").textContent = mode === "register" ? "Kayıt Ol" : "Giriş Yap";
  document.getElementById("loginTab").className = mode === "login" ? "" : "secondary";
  document.getElementById("registerTab").className = mode === "register" ? "" : "secondary";
}
...
async function submitAuth() {
  try {
    ...
    if (!username || !password) return authError("Kullanıcı adı ve şifre gir.");
    if (authMode === "register" && !name) return authError("Ad soyad gir.");
    const email = usernameToEmail(username);
    if (authMode === "register") {
      const cred = await firebase.auth().createUserWithEmailAndPassword(email, password);
      await createOrUpdateUserProfile(cred.user, { adSoyad: name, role: "personel", blocked: false, deleted: false });
    } else {
    ...
    const map = {
      ...
      "auth/email-already-in-use": "Bu kullanıcı adı daha önce alınmış.",
...
async function resetPasswordByUsername() {
  ...
  } catch (e) { authError("Şifre sıfırlama gönderilemedi. Bu yapıda şifreyi Firebase Console üzerinden de yenileyebilirsin."); }
}
...
async function createOrUpdateUserProfile(user, extra = {}) {
  ...
  const base = {
    ...
    email: user.email,
    username,
    adSoyad: extra.adSoyad || username,
    role: user.email === ADMIN_EMAIL || username === ADMIN_USERNAME ? "admin" : (extra.role || "personel"),
    blocked: Boolean(extra.blocked || false),
    deleted: Boolean(extra.deleted || false),
    app: "mesaiTakip",
    updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
    lastLogin: firebase.firestore.FieldValue.serverTimestamp()
  };
  if (!snap.exists) base.createdAt = firebase.firestore.FieldValue.serverTimestamp();
  await ref.set({ ...(snap.exists ? snap.data() : {}), ...base }, { merge: true });
  return (await ref.get()).data();
}

async function loadCloudRecords(uid) {
  const db = firebase.firestore();
  const ref = db.collection(APP_ROOT).doc(uid).collection("years").doc(String(CURRENT_YEAR));
  const snap = await ref.get();
...
async function saveCloudRecords() {
  ...
  await firebase.firestore().collection(APP_ROOT).doc(user.uid).collection("years").doc(String(CURRENT_YEAR)).set({
    uid: user.uid,
...
async function renderAdminPanel() {
  ...
  if (!isAdmin) return;
  const snap = await firebase.firestore().collection("users").where("app", "==", "mesaiTakip").get();
  ...
  document.getElementById("adminLastLogin").textContent = latest ? latest.toLocaleDateString("tr-TR") : "-";
...
async function adminSetRole(uid, role) {
  ...
  await firebase.firestore().collection("users").doc(uid).set({ role, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
  renderAdminPanel();
}
async function adminToggleBlock(uid, blocked) {
  await firebase.firestore().collection("users").doc(uid).set({ blocked, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
  renderAdminPanel();
}
async function adminSoftDelete(uid) {
  if (!confirm("Bu kullanıcı sistemde pasif/silinmiş işaretlensin mi?")) return;
  await firebase.firestore().collection("users").doc(uid).set({ deleted: true, blocked: true, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
  renderAdminPanel();
}
...
function initMesaiFirebase() {
  ...
  firebase.auth().onAuthStateChanged(async (user) => {
    try {
      ...
      isLoadingCloud = false;
      authError("Firebase bağlantı hatası: " + e.message);
    }
  });
}
